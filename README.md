...
* **Choropleth Visualization:** Renders a 10m resolution map where pixels are color-coded by action: **Targeted Tillage (Red)**, **Monitor (Amber)**, or **None (Green)**.

---

## Results & Demonstration Outputs
* **Precision Prescription:** The pipeline outputs a `final_payload.json` containing specific ripper depths and confidence bounds for every pixel in the field.
* **Summary Analytics:** The dashboard provides an acreage breakdown. Each pixel represents **0.0247 acres** (100m²), allowing the user to see exactly what percentage of the field requires intervention.
...
[![CompactIQ Demo](**https://i9.ytimg.com/vi/3IK-2ezO4Rg/mq2.jpg?sqp=CMCgls4G-oaymwEmCMACELQB8quKqQMa8AEB-AGwBoAC4AOKAgwIABABGFwgZShIMA8=&rs=AOn4CLCOGqInykAz6pUOrtrW4kH1GysUfw**)]((https://youtu.be/3IK-2ezO4Rg))
---
...
* NEED TO CONNECT API IN BACKEND!!
...
1.  Clone the repository and install dependencies:
    ...
    ```
2.  Configure your `config.yaml` with your CDSE OIDC credentials and API flags.

### Running the App
Launch the Flask dashboard:
```bash
flask run engineer_c/app.py
```
...
* If an API timeout occurs (>30s), the system automatically pulls physically plausible data from the `mock_data/` directory.
